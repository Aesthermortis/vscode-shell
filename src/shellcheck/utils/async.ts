...
export class Throttler<T> {
  ...
  private queuedPromise: Promise<T> | null = null;
  private queuedPromiseFactory: ITask<Promise<T>> | null = null;
    ...
    if (this.activePromise) {
      ...
      if (!this.queuedPromise) {
        const onComplete = (): Promise<T> => {
          ...
          if (!factory) {
            return Promise.reject(new Error("No queued promise factory available."));
          }

          return this.queue(factory);
        };

        this.queuedPromise = this.activePromise
...
export class Delayer<T> {
  ...
  private task: ITask<T> | null = null;
  ...
  public trigger(task: ITask<T>, delay: number = this.defaultDelay): Promise<T> {
    ...
    if (!this.completionPromise) {
      this.completionPromise = new Promise<void>((resolve) => {
        ...
        const scheduledTask = this.task;
        this.task = null;

        return scheduledTask ? scheduledTask() : (undefined as unknown as T);
      });
    }
...
export class ThrottledDelayer<T> extends Delayer<Promise<T>> {
  private readonly throttler = new Throttler<T>();
