...
class JsonParserMixin {
  ...
  protected makeDiagnostic(problem: ShellCheckProblem): vscode.Diagnostic {
    ...
    diagnostic.code = {
      ...
      target: vscode.Uri.parse(`https://www.shellcheck.net/wiki/SC${problem.code}`),
    };
...
/**
 * Maps the ShellCheck severity string to the VS Code diagnostic severity.
 * @param level Severity value reported by ShellCheck.
...
const codeToTags = new Map<number, vscode.DiagnosticTag[]>([
...
/**
 * Returns the diagnostic tags associated with a ShellCheck rule, when available.
 * @param code Numeric ShellCheck rule identifier.
 * @returns The diagnostic tags for the rule, if any.
 */
function scCodeToDiagnosticTags(code: number): vscode.DiagnosticTag[] | undefined {
  return codeToTags.get(code);
}

/**
 * Creates a parser instance that matches the detected ShellCheck output format.
 * @param textDocument Document being linted.
 * @param options Parser configuration supplied by the linter.
 * @returns A parser compatible with the current ShellCheck output.
 */
export function createParser(textDocument: vscode.TextDocument, options?: ParserOptions): Parser {
  if (options && options.toolVersion && semver.gte(options.toolVersion, MINIMUM_TOOL_VERSION)) {
